...
{
    msg(D_STREAM_DEBUG, "STREAM: RESET [%d]", sb->indx);
    sb->residual_fully_formed = false;
    sb->buf = sb->buf_init[sb->indx];
    ASSERT(buf_init(&sb->buf, 0));
    sb->len = -1;
    sb->indx = ((sb->indx + 1) % 2);
}
...
stream_buf_set_next(struct stream_buf *sb)
{
    ...
    sb->next.len = sb->maxlen - sb->buf.len;
