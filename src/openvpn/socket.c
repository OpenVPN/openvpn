...
stream_buf_get_next(struct stream_buf *sb)
{
    ...
    next.len = sb->maxlen - next.offset;
    msg(D_STREAM_DEBUG, "STREAM: SET NEXT, buf=[%d,%d] next=[%d,%d] len=%d maxlen=%d", sb->buf.offset, sb->buf.len, next.offset, next.len, sb->len, sb->maxlen);
    ASSERT(next.len > 0);
...
link_socket_read_tcp(struct link_socket *sock, struct buffer *buf)
{
    ...
    struct buffer frag = stream_buf_get_next(sb);
    ...
    if (!sb->residual_fully_formed && BLEN(&frag) > low)
    {
        ...
        msg(D_STREAM_DEBUG, "STREAM: READ %d buf=%d frag=%d", (int)len, BLEN(&sb->buf), BLEN(&frag));
...
link_socket_write_tcp(struct link_socket *sock, struct buffer *buf, struct link_socket_actual *to)
{
    ...
    msg(D_STREAM_DEBUG, "STREAM: WRITE %u offset=%d", len, buf->offset);
