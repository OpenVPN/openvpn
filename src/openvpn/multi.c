...
struct multi_context *multi_context_switch_conn(struct thread_pointer *a, struct multi_context *m, struct multi_instance *i)
{
    ...
    int indx = -1, fidx = 0;
    time_t last = time(NULL), secs = time(NULL);
    ...
    if (indx < 0)
    {
        ...
        ma = &(a->p->a[indx]);
        bzero(ma, sizeof(struct multi_address));
        strncpy(ma->wans, uniq, 48);
        pthread_mutex_init(&(ma->lock), NULL);
    }
...
multi_create_instance(struct thread_pointer *a, const struct mroute_addr *real, struct link_socket *sock)
{
    ...
    mi->mtio_vlen = 0;
    bzero(&(mi->mtio_vadr), MAX_THREADS * sizeof(in_addr_t));
...
bool multi_context_switch_addr(struct multi_context *m, struct multi_instance *i)
{
    ...
    in_addr_t b, a = i->context.c2.push_ifconfig_local;
    int z = -1;
    ...
    if (strcmp(lans_addr, "") != 0)
    {
        for (int x = 0; x < m->mtio_info.maxc; ++x)
        {
            ...
            {
                pthread_mutex_lock(&(m->mtio_info.addr[x].lock));
                int e = 0, f = 0, c = 0;
                for (int y = 0; y < MAX_THREADS; ++y)
                {
                    b = m->mtio_info.addr[x].ladr[y];
                    if (b == a) { e = (y + 1); }
                    const char *t = print_in_addr_t(b, IA_EMPTY_IF_UNDEF, &g);
                    if (strcmp(t, "") != 0) { c += 1; }
                    else if (f == 0) { f = (y + 1); }
                }
                if (e != 0) { f = 0; }
                else if (f == 0) { f = i->mtio_idno; }
                if (f > 0)
                {
                    m->mtio_info.addr[x].ladr[f-1] = a;
                    m->mtio_info.addr[x].last = time(NULL);
                    c += 1;
                }
                ...
                pthread_mutex_unlock(&(m->mtio_info.addr[x].lock));
            }
        }
    }
    ...
    if (z > -1)
    {
        for (int y = 0; y < MAX_THREADS; ++y)
        {
            b = m->mtio_info.addr[z].ladr[y];
            const char *t = print_in_addr_t(b, IA_EMPTY_IF_UNDEF, &g);
            if (strcmp(t, "") != 0)
            {
                if (m->mtio_info.maxt > 1) {
                    multi_learn_in_addr_t(m, i, b, -1, true);
                } else {
                    int v = -1;
                    for (int w = 0; w < i->mtio_vlen; ++w)
                    {
                        if (i->mtio_vadr[w] == htonl(b)) { v = w; }
                    }
                    if ((v < 0) && (i->mtio_vlen < MAX_THREADS)) {
                        msg(M_INFO, "TCPv4_SERVER MTIO addr [%d][%d] [%s][%s]", y, i->mtio_vlen, lans_addr, t);
                        i->mtio_vadr[i->mtio_vlen] = htonl(b);
                        i->mtio_vlen += 1;
                    }
                }
            }
...
struct multi_instance *multi_instance_check_addr(struct multi_context *m, struct mroute_addr *s)
{
    struct multi_instance *i = m->pending;
    if (m->mtio_info.maxt <= 1)
    {
        for (int x = 0; x < i->mtio_vlen; ++x)
        {
            if (i->mtio_vadr[x] == s->v4.addr)
            {
                return m->pending;
            }
        }
...
void multi_instance_learn_more(struct multi_context *m, struct mroute_addr *s)
{
    if (m->mtio_info.maxt <= 1)
    {
    ...
    if (m->pending)
    {
        ...
        if (BLEN(&c->c2.buf2) > 0)
        {
            ...
            if (TUNNEL_TYPE(m->top.c1.tuntap) == DEV_TYPE_TUN)
            {
                ...
                else if ((multi_get_instance_by_virtual_addr(m, &src, true) != m->pending) && (multi_instance_check_addr(m, &src) != m->pending))
                {
                    ...
                    else
                    {
                        ...
                        multi_instance_learn_more(m, &src);
                    }
                    c->c2.to_tun.len = 0;
                }
