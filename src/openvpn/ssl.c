...
tls_multi_init_finalize(struct tls_multi *multi, int tls_mtu)
{
    ...
    tls_session_init(multi, &multi->session[TM_INIT]);
    multi->session[TM_INIT].key[KS_MAIN].keys_reno = true;

    tls_session_init(multi, &multi->session[TM_MAIN]);
    tls_session_init(multi, &multi->session[TM_LAME]);

    tls_session_init(multi, &multi->session[TM_SERV]);
    tls_session_init(multi, &multi->session[TM_BACK]);

    tls_session_init(multi, &multi->session[TM_NOOP]);
    tls_session_init(multi, &multi->session[TM_NULL]);
...
{
    key_state_soft_reset(tls_multi, &tls_multi->session[TM_MAIN]);
}
                  ...
                  struct link_socket_info *to_link_socket_info, interval_t *wakeup)
{
    ...
    for (int i = 0; i < TM_SIZE; ++i)
    {
         ...
         * established */
        if (i == TM_INIT && ks->state == S_INITIAL && ks->keys_reno && (secs - multi->keys_last) >= 5
        ...
        if (i == TM_MAIN && ks->state == S_GENERATED_KEYS && ks->authenticated == KS_AUTH_TRUE && multi->keys_init)
        {
