...
key_state_check_auth_pending_file(struct auth_deferred_status *ads, struct tls_multi *multi, struct tls_session *session, struct key_state *ks)
{
    ...
    if (ads->auth_pending_file)
    {
        ...
        if (lines && lines->head)
        {
            ...
            {
                send_auth_pending_messages(multi, session, ks, BSTR(extra_buf), (unsigned int)timeout);
            }
        }
...
tls_authentication_status(struct tls_multi *multi)
{
    ...
    for (int i = 0; i < KEY_SCAN_SIZE; ++i)
    {
        ...
        if (TLS_AUTHENTICATED(multi, ks))
        {
            ...
            if (TM_MAIN <= ks->key_id && ks->key_id <= TM_SERV)
            {
 ...
 */
static int
verify_user_pass_script(struct tls_session *session, struct tls_multi *multi, const struct user_pass *up)
{
    struct gc_arena gc = gc_new();
    struct argv argv = argv_new();
    const char *tmp_file = "";
    int retval = OPENVPN_PLUGIN_FUNC_ERROR;
    struct key_state *ks = tls_select_encryption_key_init(multi); /* primary key */

    /* Set environmental variables prior to calling script */
    setenv_str(multi->opt.es, "script_type", "user-pass-verify");

    /* format command line */
    argv_parse_cmd(&argv, multi->opt.auth_user_pass_verify_script);

    if (multi->opt.auth_user_pass_verify_script_via_file)
    {
        struct status_output *so;

        tmp_file = platform_create_temp_file(multi->opt.tmp_dir, "up", &gc);
        if (tmp_file)
        {
    ...
    else
    {
        ...
        setenv_str(multi->opt.es, "password", up->password);
    }
    ...
    /* call command */
    int script_ret =
        openvpn_run_script(&argv, multi->opt.es, S_EXITCODE, "--auth-user-pass-verify");
    switch (script_ret)
    {
        case 0:
...
error:
    if (!multi->opt.auth_user_pass_verify_script_via_file)
    {
        setenv_del(multi->opt.es, "password");
    }
 ...
 */
static int
verify_user_pass_plugin(struct tls_session *session, struct tls_multi *multi, const struct user_pass *up)
{
    int retval = OPENVPN_PLUGIN_FUNC_ERROR;
    struct key_state *ks = tls_select_encryption_key_init(multi); /* primary key */
    ...
    /* call command */
    retval = plugin_call(multi->opt.plugins, OPENVPN_PLUGIN_AUTH_USER_PASS_VERIFY, NULL, NULL, multi->opt.es);
...
static int
verify_user_pass_management(struct tls_session *session, struct tls_multi *multi, const struct user_pass *up)
{
    ...
    if (management)
    {
        management_notify_client_needing_auth(management, ks->mda_key_id, multi->opt.mda_context, multi->opt.es);
    }
 ...
 *
 * Will set authenticated according to result of the username/password verification
 */
void
...
verify_final_auth_checks(struct tls_multi *multi, struct tls_session *session)
{
    ...
    if (ks->authenticated > KS_AUTH_FALSE && multi->opt.client_config_dir_exclusive)
    {
        struct gc_arena gc = gc_new();

        const char *cn = multi->common_name;
        const char *path = platform_gen_path(multi->opt.client_config_dir_exclusive, cn, &gc);
        if (!cn || !strcmp(cn, CCD_DEFAULT) || !platform_test_file(path))
        {
            ...
            msg(D_TLS_ERRORS,
                ...
                multi->common_name, path ? path : "UNDEF");
        }
